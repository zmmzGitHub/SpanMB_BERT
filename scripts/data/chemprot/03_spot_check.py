...
def spot_check_fold(fold):
    ...
    fname = f"data/chemprot/processed_data/standard/{fold}.jsonl"
    ...
    f_relation = f"data/chemprot/raw_data/ChemProt_Corpus/chemprot_{fold}/chemprot_{fold}_gold_standard.tsv"
    relations = pd.read_table(f_relation, header=None)
    if "standard" in f_relation:
        relations.columns = ["doc_key", "rel_category", "arg1", "arg2"]
    else:
