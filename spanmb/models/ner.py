...
from spanmb.training.ner_metrics import NERMetrics
...
class NERTagger(Model):
    ...
    def compute_ner_scores(self, span_embeddings, metadata):
    ...
    def predict_label(self, spans, span_mask, ner_scores, ner_labels, metadata):
    ...
    def forward(self,  # type: ignore
                ...
                ner_nested: torch.IntTensor = None,
        ...
        if ner_labels is not None:
            gold_ner = {"ner_labels": ner_labels, "metadata_list": metadata}
            metrics = self._ner_metrics[self._active_namespace]
            metrics(predicted_ner, gold_ner, span_mask)
            # metrics(predicted_ner, ner_labels, ner_nested)
            ner_scores_flat = ner_scores.view(-1, self._n_labels[self._active_namespace])
            ner_labels_flat = ner_labels.view(-1)
            mask_flat = span_mask.view(-1).bool()
